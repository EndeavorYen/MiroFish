        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <div v-if="project.files.length > 3" class="files-more">
              {{ t('history.filesCount', { count: project.files.length - 3 }) }}
            </div>
          </div>
          <!-- 无文件时的占位 -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-delete-section">
              <button
                ...
                :disabled="deleting"
              >
                {{ deleting ? t('history.deleting') : t('history.deleteBtn') }}
              </button>
            </div>
          </div>
        </div>
...
const handleDelete = async () => {
  ...
  const simId = selectedProject.value.simulation_id
  const displayId = formatSimulationId(simId)
  if (!confirm(t('history.deleteConfirm', { id: displayId }))) return

  try {
    deleting.value = true
    const response = await deleteSimulation(simId)
    if (response.success) {
      closeModal()
      await loadHistory()
    }
