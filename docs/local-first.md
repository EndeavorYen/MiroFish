...
| 模擬每回合 decode | 462 | **29**（6.3%） |
| 報告 decode | 4,238（ReportAgent） | **~200**（metrics） |
| VRAM（llama-server 8K/slot） | 7.2 GB | 7.2 GB |
| 有動作回合平均延遲 | 11.0 s | 12.8 s |

### 忠實度（閘門 G4，B = 本機路徑、A = LLM 路徑）

| 條件 | 結果 | 門檻 |
| --- | --- | --- |
| 每回合 decode B／A | 0.063 ✅ | ≤ 0.10 |
| 動作分布 JS | 0.035 ✅ | ≤ 0.054（= 2 × A 組間 0.027） |
| 各角色平均立場相關 | 0.891 ✅（A 組間 0.948） | ≥ 0.5 |
| 立場分布 JS | 0.044 ❌ | ≤ 0.01（2 × A 組間 0.003，下限 0.01） |
| VRAM | 7.2 GB ✅ | ≤ 10 GB |
| 每 run agent 動作數 | 137.8 對 164.6（84%） | — |
| distinct-2 | 0.371 對 0.360 | — |

結論：本機路徑在 **動作行為、各角色立場、成本** 上達標；**貼文的立場分布** 與 LLM 路徑仍有差異（強烈反對的貼文偏少：golden 2.8% 對 12.8%），因此建議：
...
| decode B／A | 0.063 ✅ | 0.078 ✅ |
| 動作分布 JS（門檻：2 × A 組間） | 0.035 ≤ 0.054 ✅ | 0.071 ≤ 0.100 ✅ |
| 每 run 動作數 B／A | 84% | 71% |
| 各角色平均立場相關（A 組間） | 0.891（0.948）✅ | 0.527（0.441）✅ |
| 立場分布 JS（A 組間） | 0.044（0.003）❌ | 0.046（0.011）❌ |

動作行為、各角色立場與成本的結果在新情境上都成立；兩個情境一致未通過的只有貼文立場分布。
