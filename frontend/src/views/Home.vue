...
// Hero description with inline HTML
const heroDescHtml = computed(() => {
  const brand = '<span class="highlight-bold">MiroFish</span>'
  const agentScale = `<span class="highlight-orange">${t('home.agentScale')}</span>`
  const optimalQuote = `<span class="highlight-code">${t('home.optimalQuote')}</span>`
  return t('home.heroDesc', { brand, agentScale, optimalQuote })
})
