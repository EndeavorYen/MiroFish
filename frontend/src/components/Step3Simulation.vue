    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? t('simRun.startingUp') : t('simRun.generateReport') }} 
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog(t('simRun.startingDualSim'))
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(t('simRun.setMaxRounds', { rounds: props.maxRounds }))
    }
    
    addLog(t('simRun.graphUpdateEnabled'))
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(t('simRun.forceRestarted'))
      }
      addLog(t('simRun.engineStarted'))
      ...
      startError.value = res.error || t('simRun.startFailed')
      addLog(t('simRun.startError', { error: res.error || t('simRun.unknownError') }))
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(t('simRun.startException', { error: err.message }))
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog(t('simRun.stoppingSimulation'))
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog(t('simRun.simulationStopped'))
      phase.value = 2
      stopPolling()
      emit('update-status', 'completed')
    } else {
      addLog(t('simRun.stopFailed', { error: res.error || t('simRun.unknownError') }))
    }
  } catch (err) {
    addLog(t('simRun.stopException', { error: err.message }))
  } finally {
    isStopping.value = false
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog(t('simRun.allPlatformsDone'))
        }
        addLog(t('simRun.simCompleted'))
        phase.value = 2
        stopPolling()
        emit('update-status', 'completed')
      }
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog(t('simRun.reportAlreadySent'))
    return
  }
  
  isGeneratingReport.value = true
  addLog(t('simRun.startingReport'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('simRun.reportTaskStarted', { reportId }))
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(t('simRun.reportStartFailed', { error: res.error || t('simRun.unknownError') }))
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(t('simRun.reportStartException', { error: err.message }))
    isGeneratingReport.value = false
  }
}
