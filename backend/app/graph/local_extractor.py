...
PERSON_TYPE_WORDS = {
    ...
    "employee", "commuter", "victim", "fan", "voter", "patient", "mayor", "minister",
    "legislator", "lawmaker", "senator", "governor", "president", "councillor",
    "councilor", "director", "manager", "founder", "owner", "witness", "commentator",
}
...
def _person_like_type(entity_type: str | None) -> bool:
    ...
    if not words:
        return False
    head = words[-1].lower()
    if head.endswith("s") and head[:-1] in PERSON_TYPE_WORDS:
        head = head[:-1]  # Officials, Students
    return head in PERSON_TYPE_WORDS or head.endswith(("person", "man", "woman", "men", "women"))
