...
def generate_report():
    ...
    try:
        ...
        regenerate_hint = data.get('regenerate_hint', '')
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "graph_id is missing, please ensure the graph has been built"
            }), 400
        ...
        # Append regeneration hint to requirement if provided
        if regenerate_hint:
            simulation_requirement = f"{simulation_requirement}\n\n[Regeneration hint: {regenerate_hint}]"
